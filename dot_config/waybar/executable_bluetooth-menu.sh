...
# Constants
# Base divider character (Box Drawings Light Horizontal U+2500)
DIV_CHAR="─"
DIV_COLOR="#4C566A"

# Helper to generate divider of specific length
get_divider() {
  local length=$1
  local chars=""
  for ((i=0; i<length; i++)); do chars="${chars}${DIV_CHAR}"; done
  echo "<span color='${DIV_COLOR}'>${chars}</span>"
}
...
show_device_info() {
  ...
  local div=$(get_divider 37) # Fits 400px
...
show_device_menu() {
  ...
  local div=$(get_divider 19) # Fits 250px
...
show_main_menu() {
  ...
  if is_powered_on; then
    ...
    local div=$(get_divider 45) # Fits 500px
    entries="${entries}\n$div\n"
  ...
  local selected=$(echo -e "$entries" |
    wofi --width 500 --height 200 --dmenu --cache-file /dev/null --allow-markup --prompt "Bluetooth" --insensitive)
  ...
  if [ -z "$selected" ]; then
    ...
    return
  elif echo "$selected" | grep -q "$DIV_CHAR"; then
    show_main_menu
  else
